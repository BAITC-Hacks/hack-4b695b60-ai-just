...
У обязательных текстов удаляются внешние пробелы до проверки длины; внутренние пробелы и переносы сохраняются. Минимум: название бизнеса — 2 символа, черновик — 10, идея и план — 20, срок — 1, название этапа — 3. В `CardPatch` значение поля ограничено 2000 символами; комментарий решения бизнеса — 500. Формы frontend используют те же лимиты.
...
В новых отчётах указана `prompt_version`. Для v2 `final_guard_failure_rate` — повторная структурная проверка после фильтрации (не оценка смысловой достоверности); `semantic_hallucination_rate` равен `null`, пока независимая оценка не выполнена. Старое имя `final_hallucination_rate` больше не записывается. Сохранённые отчёты v1 исторические и не описывают текущие промпты/grounding. Если JSON-отчёт ещё не создан запуском eval, endpoint возвращает 404.
