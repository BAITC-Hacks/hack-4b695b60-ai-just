...
Регистрацию и пароли (вместо них переключатель ролей), realtime, уведомления, файловое хранилище, векторную БД (numpy в памяти), обучение своих моделей (берём готовые), миграции БД (`create_all`; если в таблице не хватает колонок из моделей, БД при старте пересоздаётся из seed), мобильную вёрстку, прод-деплой.
