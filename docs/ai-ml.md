...
| 8 | **Переключение провайдера на лету**: OpenAI, NVIDIA, Brev, офлайн | Демо | P1 |
...
| 12 | Модерация публичных текстов (NVIDIA NemoGuard) | Шаги 5–6 | P2 |
...
| `nvidia` | `nvidia/nemotron-3-nano-30b-a3b` (быстрый MoE, 3B активных параметров); для качества — `nvidia/nemotron-3-super-120b-a12b` | `nvidia/nemotron-3-embed-1b` (34 языка, включая русский) | Второй в цепочке; эмбеддинги NVIDIA идут первыми |
| `brev` | Свой Nemotron 3 Nano на GPU NVIDIA Brev через vLLM или NIM | — | Приватный режим: данные не уходят третьим лицам |
| `stub` | Правила и банк вопросов | TF-IDF (char n-grams) | Всегда последний, работает без сети |

Важные детали:
- **Build API для +7 недоступен.** Ключ выдаётся после SMS-верификации, а Казахстана нет в списке стран (подробности — в `docs/nvidia-brev.md`). Без ключа провайдер `nvidia` просто пропускается, и NVIDIA в проекте представлена через `brev`: Nemotron на своём GPU.
- **NVIDIA Build** — это OpenAI-совместимый API на `https://integrate.api.nvidia.com/v1` с ключом `nvapi-...`. У Nemotron 3 рассуждение включено по умолчанию. Для быстрого JSON отключаем его: `extra_body={"chat_template_kwargs": {"enable_thinking": False}}`.
- **Nemotron 3 Nano дообучали на английском, испанском, французском, японском, итальянском и немецком. Русский был только в предобучении.** Поэтому для русских черновиков основной провайдер — OpenAI, а NVIDIA идёт вторым. Порядок подтверждаем метриками eval (раздел 10).
- Старый эндпоинт эмбеддингов `llama-3.2-nv-embedqa-1b-v2` на build.nvidia.com помечен как устаревший. Используем `nvidia/nemotron-3-embed-1b`. У моделей NVIDIA обязателен `input_type`: `passage` для задач, `query` для профиля команды.
- Все ID моделей задаются в `.env`. Если модель пропала из каталога, меняем переменную, код не трогаем.
...
- `AI_PROVIDER=auto` идёт по цепочке. `openai`, `nvidia`, `brev` или `stub` означают только этот провайдер и заглушку как страховку.
...
| NVIDIA Build | `chat.completions.create(..., response_format={"type": "json_object"})` или `extra_body={"nvext": {"guided_json": schema}}`, если модель поддерживает. Если 400 — убрать параметр и полагаться на промпт и валидацию |
| Brev: vLLM | `response_format={"type": "json_schema", "json_schema": {"name": ..., "schema": ...}}` |
| Brev: NIM | `extra_body={"nvext": {"guided_json": schema}}` |
...
- Цепочка эмбеддингов `EMBED_PROVIDER_CHAIN=nvidia,openai,tfidf`. TF-IDF: `TfidfVectorizer(analyzer="char_wb", ngram_range=(3, 5))`, обучается на текстах задач и команды. Работает на русском без загрузки моделей.
- Итоговая оценка:
...
- `run_eval.py`: `uv run python -m eval.run_eval --providers stub,openai,nvidia[,brev] --out eval/reports/`
...
- **Модерация:** `nvidia/llama-3.1-nemoguard-8b-content-safety` на build.nvidia.com (наличие проверить в каталоге) при публикации и отклике. Небезопасный текст блокируется с понятным сообщением.
...
4. Переключение провайдера: OpenAI → NVIDIA → офлайн, сценарий не ломается.
