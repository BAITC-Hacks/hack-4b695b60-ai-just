...
У предложения AI должна быть цитата из черновика или ответа. Проверка grounding ищет совпадение цитаты, сверяет числа, email, URL и @-ники с источником, отбрасывает неподтверждённые предложения и записывает причину в трассировку. Это набор автоматических проверок, который может пропустить смысловую ошибку; перед публикацией человек проверяет карточку.

Email, телефоны и @-ники маскируются перед внешним LLM. Для рекомендаций используются эмбеддинги OpenAI или локальный TF-IDF вместе с совпадением навыков. Brev — облачная GPU-инфраструктура для самостоятельного запуска модели; используйте синтетические данные и не считайте такой запуск гарантией, что данные остаются на вашем компьютере.
...
В [сохранённом отчёте](backend/eval/reports/latest.md) сравниваются 12 размеченных синтетических черновиков, включая короткие описания, контакты и попытки подменить инструкции. В указанном прогоне каждый провайдер действительно ответил на 12 из 12 примеров, без подмены результата fallback-провайдером.
...
**Как читать результаты:** Field F1 измеряет обнаружение размеченных типов полей без `title`, а не смысловую точность всего текста. Показатель `final_hallucination_rate=0` в отчёте означает, что оставшиеся поля прошли повторную проверку тем же grounding-кодом; это не независимая экспертная оценка достоверности. Малый синтетический набор не позволяет переносить эти числа на любые реальные задачи. Стоимость запросов стенд пока не рассчитывает.
...
**Терминал 2**, из корня: сначала остановите обычный Vite на порту 5173, чтобы тест запустил свой frontend с нужным proxy. Используется Google Chrome; если он ещё не установлен, выполните `npx playwright install chrome` из `frontend/`.
